...
function cityGuide(
  ...
  return article(
    ...
    `<p><strong>${city} matters to Street Collector because the roster includes artists whose work gives collectors a real way into the scene.</strong> This guide is not a tourist mural map. It is a collector's guide to artists, profiles, and prints connected to the city.</p>
<blockquote><p><strong>At a glance:</strong> Use linked roster profiles below—collector intel, not mural tours—and confirm editions on Shopify-backed product cards before you buy. Branch out via <a href="/shop/explore-artists">artist directory</a>, <a href="/urban-art-prints">urban art prints</a>, <a href="/shop/products">available artworks</a>.</p></blockquote>
<p>${sceneAngle}</p>
<h2>Street Collector artists to know</h2>
${artistParagraphs}
<h2>How to collect from this scene</h2>
<p>Start with artist pages, not generic city claims. Read the profile, check current works, and compare how each artist's visual language translates into a limited edition print.</p>
...
<p>No. It is a collector-focused guide anchored to artists in the Street Collector roster.</p>
...
<blockquote><p><strong>At a glance:</strong> Limited = declared cap on the run; numbering (e.g. 12/50) IDs your sheet inside that cap. Always confirm signing, paper, and COA on the live <a href="/shop/products">product page</a>—this article frames concepts, not a specific SKU.</p></blockquote>
...
<p>Street Collector focuses on limited edition artist releases. Eligible works include Certificate of Authenticity documentation.</p>`,
  ...
  article(
    ...
    `<p><strong>You start by backing a person you want to follow—not a trend, not a random grid pick.</strong> Street art prints reward collectors who read the artist profile, understand the edition line, and route the work into a display habit (frame, shelf, or Street Collector's backlit, swappable setup).</p>
<blockquote><p><strong>Key takeaways:</strong> Open the artist page before the cart. Read total edition size and numbering alongside price. Decide where the work lives in your space so buying is not postponed by layout anxiety. Bundle receipts, Certificate of Authenticity paperwork, and product URLs in one folder.</p></blockquote>
...
<h3>Famous versus emerging—which makes sense?</h3>
<p>Neither label alone wins. Emerging names can deepen your relationship with studio updates; marquee names narrow liquidity risk but rarely remove homework. Decide based on the art, editions, storytelling, fiscal comfort.</p>
...
<blockquote><p><strong>At a glance:</strong> Poster = volume decor; artist-led print = finite run + story. Use <a href="/shop/explore-artists">artist context</a> plus <a href="/shop/products">edition fields</a> to avoid grid confusion.</p></blockquote>
...
<blockquote><p><strong>At a glance:</strong> Read X/Y as &quot;this sheet / total run&quot;; lower X is preference, not automatic premium. Cross-check the live listing for AP/PP callouts.</p></blockquote>
...
<blockquote><p><strong>At a glance:</strong> Anchor taste to a known artist/style, bundle <a href="/backlit-art-lamp">lamp</a> + inaugural <a href="/shop/products">edition</a> when you want room to grow gifts over time—not a locked frame story.</p></blockquote>
...
<p>Yes. It gives the person a display object and a first artwork without requiring a large wall or custom framing.</p>`,
  ...
  article(
    ...
    `<p><strong>Tel Aviv's Street Collector roster is strongest where street art, illustration, poster culture, typography, and character work overlap.</strong> This is not a tourist mural list. It is a collector's entry point into artists connected to the city and wider Israeli visual culture through actual Street Collector profiles.</p>
<blockquote><p><strong>At a glance:</strong> Dive through linked roster names (Toor/Yonil/Shtai/etc.) toward shoppable listings—coverage reflects published profiles at publish time; availability moves with drops.</p></blockquote>
<p>The useful question is not only where to photograph street art in Tel Aviv. For collectors, the better question is: which artists have a visual language that can move from street, studio, poster, screen, and print into something you can own?</p>
...
<p><a href="/shop/artists/ori-toor">Ori Toor</a> is the clearest first anchor: Tel Aviv-based, dense, improvisational, and especially strong for collectors who like prints that keep unfolding the longer you look.</p>
<p><a href="/shop/artists/yonil">Yonil</a> is useful for understanding the poster and typography side of the city: music culture, type, and graphic immediacy rather than polite gallery distance.</p>
...
<p>Street Collector gives the city guide a practical pullback: artist profiles, limited edition works, and a display system for rotating the collection. Start with the <a href="/shop/explore-artists">artist directory</a>, browse <a href="/urban-art-prints">urban art prints</a>, or open <a href="/shop/products">current artworks</a>.</p>
<h2>FAQ</h2>
...
<p>You can browse available works through each artist page on Street Collector. Product pages show current availability, prices, and edition context.</p>`,
  ...
  article(
    ...
    ['Artist Spotlights', 'Limited Editions', 'Street Collector Artists'],
    `<p><strong>Ori Toor is a Tel Aviv–based artist in the Street Collector roster who builds images by staying inside the drawing gesture until it forks into its own ecosystem.</strong> What looks like controlled chaos is disciplined mark-making: improvisation that still produces a legible world you can recognize from across a room, then interrogate up close.</p>
<blockquote><p><strong>Key takeaways:</strong> Treat his prints like time-lapse stills—each pass adds fauna, pattern, or motion logic. Backlighting emphasizes the chromatic rhythm Street Collector lamps are built to highlight. Anchor collecting on the artist page so edition notes and studio context stay attached to the physical work.</p></blockquote>
...
<p>Large-scale digital pieces can collapse when reduced. Toor's layering strategy—alternating tight hatching with open breathing room—means a print still telegraphs motion at desk scale and keeps revealing micro-decisions when you lean in. That reward loop is textbook collection behavior: the object should still teach you something after week six on your wall or lamp.</p>
...
<p>Because the marketplace keeps the work connected to provenance, artist context, and a display system built for repeat collecting instead of stopping at checkout.</p>`,
  ...
  artistSpotlight(
    ...
    'Dawal prints preserve that sense of discovery. They let collectors bring the wall-based logic into a domestic object while keeping the connection to Paris surfaces, scale shifts, and urban interruption.',
    'dawal',
  ),
  artistSpotlight(
    ...
    'Maalavidaa prints are strong candidates for illuminated display because backlighting can amplify the gradients, contrast, and atmospheric shifts that already sit at the center of the work.',
    'maalavidaa',
  ),
  artistSpotlight(
    ...
    'Her work moves between editorial illustration, animation, and personal image-making. The strongest pieces have a dreamlike stillness, but they are not vague; they are composed, restrained, and specific about feeling.',
...
<blockquote><p><strong>At a glance:</strong> Treat the denominator in X/Y as the print run anchor; carve-outs like AP still need SKU copy. Tie back to our <a href="/shop/blog/what-is-a-limited-edition-print">limited edition explainer</a> before dropping money.</p></blockquote>
...
<p>Yes. It is useful for renters and small spaces because one display can support multiple artworks without repeated framing or wall changes.</p>`,
  ),
  article(
...
<blockquote><p><strong>At a glance:</strong> Decide on artist-image-doc fit first, treat resale as uncertain-no platform promises upside. Sanity-check comps through <a href="/shop/products">live listings</a>, not anecdotes.</p></blockquote>
...
<p>Start with the artist. Does the work have a recognizable visual language? Is the edition finite? Are the product details clear? Is the price reasonable for the artist, format, and release? Is there documentation you can keep with the work?</p>
...
<p>Editioning gives new collectors a rational way to start. It creates a finite object, connects the work to an artist page, and gives the buyer a clearer record than a generic poster.</p>
...
<blockquote><p><strong>At a glance:</strong> Labels matter less than proof: who authored it, how finite the run is, what ships with the parcel. Explore <a href="/urban-art-prints">urban art prints hub</a> when you care about lineage + edition clarity.</p></blockquote>
...
<p>It can be. Street art becomes collectible when the artist, visual language, documentation, and market context are strong enough to survive outside the wall where the work first appeared.</p>
...
<p>Sometimes. The strongest street art can be collected, exhibited, and discussed as fine art, especially when artist context and documentation are clear.</p>
...
<p>Street art prints can be a good entry point when the edition is clear and the artist context is strong.</p>`,
  ),
  article(
    ...
    `<p><strong>Street Collector merges finite artist editions, transparent roster storytelling, Certificates of Authenticity coverage where offered, and a reusable illuminated fixture.</strong> Mass poster shops optimize for SKU breadth; Street Collector optimizes for collection continuity—you should feel who made the piece and how scarcity is communicated.</p>
<blockquote><p><strong>At a glance:</strong> Poster marketplaces widen SKUs—we tighten story + edition ergonomics (+ optional <a href="/backlit-art-lamp">lamp ecosystem</a>). Use the comparison table plus live <a href="/shop/products">inventory</a> when deciding.</p></blockquote>
<h2>At-a-glance positioning</h2>
<table><thead><tr><th scope="col">Buyer question</th><th scope="col">Poster-first marketplace</th><th scope="col">Street Collector</th></tr></thead><tbody><tr><th scope="row">Primary promise</th><td>Open SKUs &amp; price breadth</td><td>Editioned indie releases tied to biography</td></tr><tr><th scope="row">Artist storytelling</th><td>Thin or vendor-dependent</td><td>Centralized artist hub + editorial blog linkage</td></tr><tr><th scope="row">Display ergonomics</th><td>Ship-and-frame yourself</td><td>Optional backlit interchangeable module</td></tr><tr><th scope="row">Trust artefacts</th><td>Varies wildly</td><td>Edition fields + documented COA where eligible</td></tr></tbody></table>
<h2>What should you investigate?</h2>
<p>Compare roster curation ethos, scarcity language, turnaround times, refunds, resale policies, framing costs, tariffs. Price per square inch rarely tells enough when authenticity matters.</p>
<h2>When Street Collector is the sharper tool</h2>
<p>If you crave artist alignment, repeatable buying rituals, rotational display without new hardware each drop, collector education (<a href="/shop/blog/how-to-start-collecting-street-art-prints">beginner playbook</a>, <a href="/shop/blog/what-is-a-certificate-of-authenticity-for-art">COA briefing</a>), this lane wins.</p>
<h2>When another vendor still fits</h2>
<p>Need giant format wallpaper, templated typography gifts, aggressively cheap decor? Big-box print APIs remain appropriate—just differentiate them psychologically from what you knowingly collect versus stage temporarily.</p>
<p>Surf inventory via <a href="/shop/products">available artworks</a>, <a href="/shop/explore-artists">artists index</a>, or <a href="/backlit-art-lamp">fixture overview</a> before deciding.</p>
<h2>FAQ</h2>
...
<p>It is both: a physical art display system and a platform for collecting limited edition prints from independent artists.</p>`,
  ...
  artistSpotlight(
    ...
    'Dima Korma prints support the Berlin street-art cluster and work well for collectors who want urban abstraction rather than a single character or slogan.',
    'dima-korma',
  ),
  artistSpotlight(
    ...
    'Studio Giftig',
    'Netherlands',
    'Studio Giftig is known for cinematic mural realism: figures, atmosphere, and large-scale painted scenes with a strong sense of drama.',
  ...
  artistSpotlight(
    ...
    'Nia Shtai',
    'Tel Aviv, Israel',
    'Nia Shtai builds saturated character work with emotional color, digital polish, and the feeling of images that want to keep moving.',
    'Her visual language sits between illustration, animation, and expressive portraiture. Color does a lot of the emotional work, but the characters keep the image grounded.',
    'Nia Shtai prints are a natural fit for illuminated display because strong color and character clarity hold up in the Street Collector lamp format.',
    'nia-shtai',
  ),
  artistSpotlight(
    ...
    'Erezoo',
    'Haifa, Israel',
    'Erezoo connects illustration, design, and graffiti influence into a practice that treats characters and marks as part of the same visual language.',
    ...
    'Erezoo prints support the Haifa and Israeli artist cluster while giving collectors a street-influenced entry point with a softer illustrative edge.',
    'erezoo',
  ),
  artistSpotlight(
    ...
    'Her practice brings a painterly, body-aware perspective into a roster often associated with graphic street energy. That contrast is useful: it widens what Street Collector can mean.',
  ...
  artistSpotlight(
    ...
    'The practice is useful for Street Collector because it bridges image and interface: visual rhythm, type, and motion thinking all appear in the work.',
    "Unapaulogetic prints support the platform's more digital and design-led side while still connecting back to Tel Aviv visual culture.",
    'unapaulogetic',
  ),
  artistSpotlight(
    ...
    'The work rewards close looking. It is precise, patient, and visually crowded in the best way: every section feels intentionally drawn.',
  ...
  artistSpotlight(
    ...
    'Jerome Masi prints give Street Collector a calmer French illustration angle and a useful counterweight to louder urban art pieces.',
    'jerome-masi',
  ),
  article(
...
<p>Where Korma leans gritty, NASCA sharpens symbolism and pacing for collectors migrating from graf books to editions. Useful if you oscillate between graphic novels and tactile painting.</p>
  ...
  cityGuide(
    ...
    [
      ...
      },
      {
        name: 'Marylou Faure',
        slug: 'marylou-faure',
        angle: 'London-based character and pop illustration with a bold, collectible visual language.',
      },
      {
        name: 'Linda Baritski',
        slug: 'linda-baritski',
      ...
      },
      {
        name: 'Tania Yakunova',
        slug: 'tania-yakunova',
        angle: 'Illustration-led work that gives collectors another route into London visual culture.',
      },
    ],
  ),
  cityGuide(
    ...
    [
      {
        name: 'Rik Lee',
        slug: 'rik-lee',
        angle: 'Melbourne-based illustration with a personal, hand-drawn language that works well at collectible print scale.',
      },
      {
  ...
  cityGuide(
    ...
    [
      ...
      },
      {
        name: 'Studio Giftig',
        slug: 'studio-giftig',
        angle: 'Dutch mural realism with cinematic scale and strong collectible image presence.',
      },
    ],
  ),
  article(
    ...
    `<p><strong>Some street-influenced artists translate especially well into small-format prints because their work has clear composition, strong rhythm, and enough detail to reward repeat viewing.</strong> This is not a popularity ranking. It is a collector's argument about scale.</p>
<blockquote><p><strong>At a glance:</strong> Composition &gt; billboard scale—grab samples from linked artist URLs, inspect zoom crops on PDP photography before assuming mural energy survives desk distance.</p></blockquote>
<h2>What makes an artist work well small?</h2>
<p>Look for a strong silhouette, controlled color, readable structure, and detail that survives reduction. A mural can overwhelm a wall and still fail as a print. A print needs its own internal logic.</p>
<h2>Street Collector picks</h2>
<p><a href="/shop/artists/ori-toor">Ori Toor</a> works small because the density keeps unfolding. <a href="/shop/artists/hedof">Hedof</a> works small because the graphic warmth stays clear. <a href="/shop/artists/moritz-adam-schmitt">Moritz Adam Schmitt</a> works small because the vector force reads quickly. <a href="/shop/artists/loreta-isac">Loreta Isac</a> works small because the emotional tone is precise. <a href="/shop/artists/iain-macarthur">Iain Macarthur</a> works small because detail becomes the reward.</p>
...
<p>No. Some wall work depends on scale, while some artists have a visual language that remains strong in print form.</p>
<h3>What should I look for in a small print?</h3>
<p>Look for clear composition, strong color, readable detail, and an image you still want to inspect after the first glance.</p>
...
<blockquote><p><strong>At a glance:</strong> Stitch cities through intentional artist pairs (see exemplars below)—always finish on <a href="/shop/products">shoppable PDPs</a> so geography isn&apos;t hypothetical.</p></blockquote>
...
<p>Use the <a href="/shop/explore-artists">artist directory</a> for discovery and <a href="/shop/products">available artworks</a> for buying. The Street Collector lamp lets the visible work rotate while the collection grows.</p>
<h2>FAQ</h2>
<h3>Can I collect street art by city?</h3>
<p>Yes. City clusters are a useful way to build a collection with geographic and cultural range.</p>
...
<p>Start with the artist, then use city as a way to give your collection structure.</p>`,
  ...
  article(
    ...
    `<p><strong>Street art can work in a minimalist interior when the print has strong composition, controlled color, and enough negative space to breathe.</strong> The goal is not to make street art polite. The goal is to choose work that holds tension without visual clutter.</p>
<blockquote><p><strong>At a glance:</strong> Favor tonal restraint or disciplined geometry—even loud palettes work when shapes stay ordered. Rotate via <a href="/interchangeable-art-prints">interchangeable prints</a> so the calm room survives taste shifts.</p></blockquote>
<h2>What to look for</h2>
<p>Choose pieces with a limited palette, clean structure, or one strong focal point. Minimal rooms can handle bold art, but they punish messy composition.</p>
...
<p>A single illuminated display can become the one strong object in a restrained room. With <a href="/interchangeable-art-prints">interchangeable prints</a>, the room stays calm while the collection changes.</p>
<h2>FAQ</h2>
...
<p>Start with artists whose work has clear composition, then check current products and edition details.</p>`,
  ...
  article(
    ...
    `<p><strong>Bold rooms need street art prints that can hold their own: saturated color, dense composition, strong characters, or graphic contrast.</strong> Maximalist spaces reward work with visual confidence.</p>
<blockquote><p><strong>At a glance:</strong> Density still needs backbone—avoid mud by picking artists with repeatable focal geometry. Amplify backlight via <a href="/backlit-art-lamp">lamp saturation</a> when color is the storyline.</p></blockquote>
...
<p>A good bold print has strong color or contrast, but also enough composition to stay readable.</p>
...
<p>Do not buy because someone promises future value. Buy because the work is strong, the edition is clear, and you want it in your collection.</p>
<h2>FAQ</h2>
...
<blockquote><p><strong>At a glance:</strong> Acid-free hinge systems + UV glazing when sun hits shelves; renters or rotators leverage <a href="/backlit-art-lamp">illuminated trays</a> instead of punching more holes.</p></blockquote>
...
<p>Use the lamp when you want a swappable display for multiple editions, when you have limited wall space, or when the work benefits from backlighting. Browse <a href="/backlit-art-lamp">the backlit art lamp</a> and <a href="/interchangeable-art-prints">interchangeable prints</a>.</p>
<h2>FAQ</h2>
<h3>Should art prints be framed under glass?</h3>
<p>Often yes, especially if the print needs protection from dust, handling, and light exposure. Use archival materials where possible.</p>
...
<p>It can serve as a display alternative for compatible Street Collector prints, especially when you want rotation rather than a fixed framed piece.</p>`,
  ),
  article(
    ...
    `<p><strong>The Street Collector Watchlist is a way to follow artworks or artists when you are not ready to buy, when an edition sells out, or when you want to track future availability.</strong> It turns browsing into a collector signal.</p>
<blockquote><p><strong>At a glance:</strong> Saves don&apos;t hold inventory-queue checkout normally still obeys first-paid rules. Combine watchlist pings with PDP alerts/email habits so sellouts don&apos;t blindside you.</p></blockquote>
...
<p>Both can be useful. Watch products for availability and artists for future releases.</p>`,
  ),
  article(
...
<blockquote><p><strong>At a glance:</strong> Narrate why the artist resonates, cross-check editions on the PDP, solve display (<a href="/backlit-art-lamp">lamp</a>, frame, or flat file) before debating clout benchmarks.</p></blockquote>
<h2>Start with artist fit</h2>
<p>Open the artist page. Read the profile. Look at the visual language across more than one work. If you can explain why the artist interests you, you are closer to a good first choice.</p>
