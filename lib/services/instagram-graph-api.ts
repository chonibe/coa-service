...
export async function getInstagramMedia(
  ...
  try {
    console.log(`Fetching Instagram media for business ID: ${igBusinessId.substring(0, 5)}...`)

    const url = `${GRAPH_API_BASE}/${igBusinessId}/media?fields=id,caption,media_type,media_url,permalink,thumbnail_url,timestamp&limit=${limit}&access_token=${accessToken}`
    console.log(`API URL: ${url.substring(0, 60)}...`)
...
export async function getInstagramCredentials(artistId: string): Promise<any | null> {
  try {
    // Use the actual environment variables instead of mock values
    return {
      artist_id: artistId,
