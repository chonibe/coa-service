...
export async function getInstagramStories(igBusinessId: string, accessToken: string): Promise<InstagramMedia[]> {
  try {
    ...
    try {
      ...
      // If we get a 400 error, it might be because there are no stories
      // or the endpoint is not available for this account
      if (!response.ok) {
        const errorText = await response.text()
        console.warn(`Instagram stories API error: ${response.status}`, errorText)
