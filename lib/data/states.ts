/**
 * States/provinces by country for address forms.
 * Used to filter state dropdown by selected country.
 */
...
/** US states (2-letter postal codes) */
...
/** Countries that have a state/province dropdown (vs free text) */
...
export function countryHasStates(countryCode: string): boolean {
  return (STATES_BY_COUNTRY[countryCode]?.length ?? 0) > 0
}
