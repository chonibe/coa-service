-- Migration: Exclude restocked and cancelled items from edition assignment
-- This migration updates the assign_edition_numbers function to explicitly ignore items 
-- from orders that are restocked, canceled, refunded, or voided.
...
BEGIN
    ...
    -- BUT only for orders that are still valid (not restocked/canceled/refunded)
    ...
    AND (
        li.nfc_claimed_at IS NULL -- Reset non-authenticated
        OR o.fulfillment_status IN ('restocked', 'canceled') -- Reset if invalid order
    ...
    LOOP
        ...
        -- 3. Check Warehouse cache as fallback
