...
BEGIN
    ...
    SELECT COUNT(*) INTO oli_count FROM order_line_items_v2;
    SELECT COUNT(*) INTO products_count FROM products;
    
    -- Count matching records
    SELECT COUNT(*) INTO match_count 
    FROM order_line_items_v2 oli
    JOIN products p ON oli.product_id::text = p.id::text
    WHERE oli.img_url IS NULL
    AND p.image_url IS NOT NULL;
    
    -- Log the counts
    RAISE NOTICE 'Total order_line_items_v2 records: %', oli_count;
