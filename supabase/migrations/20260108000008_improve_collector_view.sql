-- Migration: Improve Collector View and Backfill Emails
-- 1. Backfill orders.customer_email from raw Shopify data where missing
UPDATE public.orders 
SET customer_email = LOWER(raw_shopify_order_data->'customer'->>'email')
WHERE customer_email IS NULL 
AND raw_shopify_order_data->'customer'->>'email' IS NOT NULL;

-- 2. Backfill orders.customer_email from warehouse_orders where still missing
UPDATE public.orders o
SET customer_email = LOWER(wo.ship_email)
FROM public.warehouse_orders wo
WHERE (LOWER(o.order_name) = LOWER(wo.order_id) OR o.id = wo.shopify_order_id)
AND o.customer_email IS NULL
AND wo.ship_email IS NOT NULL;

-- 3. Backfill line items owner_email from parent orders
UPDATE public.order_line_items_v2 oli
SET owner_email = LOWER(o.customer_email)
FROM public.orders o
WHERE oli.order_id = o.id
AND oli.owner_email IS NULL
...
SELECT
  ...
  -- Statistics (counting editions by checking line item owner OR parent order customer)
  (
    SELECT COUNT(oli.id) 
    ...
    WHERE 
      (
        ...
        OR (oli.owner_email IS NULL AND LOWER(o.customer_email) = cb.email)
      ) 
