-- Migration: Robust Type Handling in assign_edition_numbers
-- Fixes type mismatch errors when comparing product IDs
...
BEGIN
    ...
    -- Use robust casting for product_id comparison
    SELECT 
        edition_size,
    ...
    LOOP
        -- Skip if this item is already authenticated
        ...
        -- Backfill email from orders table
        ...
        IF v_order_customer_email IS NULL OR v_order_customer_email = '' THEN
            ...
            FROM orders
            WHERE id::text = line_item.order_id::text; -- Added casting here
        END IF;
