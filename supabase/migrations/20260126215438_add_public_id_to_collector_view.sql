-- Migration: Add public_id to collector_profile_comprehensive view
-- This adds a secure, non-reversible identifier to replace email in URLs
-- Security fix: Prevents PII exposure in URLs
...
SELECT
  ...
  -- SECURITY FIX: Add public_id for URLs (SHA-256 hash of email)
  ...
  -- Associated Order Names (for robust activity fetching)
  (
    SELECT array_agg(DISTINCT order_name)
    FROM orders
    WHERE LOWER(customer_email) = cb.email
  ) as associated_order_names,
