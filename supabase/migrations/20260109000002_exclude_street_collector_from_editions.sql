-- Migration: Exclude Street Collector items from edition number assignment
-- Street Collector items (shipping protection, kits, etc.) should not receive edition numbers

-- 1. Update the assign_edition_numbers function
...
BEGIN
    ...
    -- BUT EXCLUDE Street Collector (they shouldn't have been authenticated anyway)
    ...
    -- Reset numbers for non-authenticated OR Street Collector items
    ...
    FOR line_item IN 
        SELECT li.id, li.order_id::TEXT as order_id_text, li.line_item_id, li.created_at, li.nfc_claimed_at, li.edition_number, li.owner_name, li.owner_email, li.customer_id, li.owner_id, li.shopify_customer_id, li.vendor_name, o.order_name
    ...
    LOOP
        -- Skip if NFC already claimed (and not a Street Collector item which we reset above)
        IF line_item.nfc_claimed_at IS NOT NULL AND line_item.edition_number IS NOT NULL THEN
            CONTINUE;
        END IF;

        -- EXPLICIT SKIP for Street Collector
        IF LOWER(COALESCE(line_item.vendor_name, '')) IN ('street collector', 'street-collector') THEN
            CONTINUE;
        END IF;
        ...
        BEGIN
            ...
            SET 
                ...
                customer_id = v_supabase_user_id, -- Sync for RLS policies
...
BEGIN
    ...
    -- 3. Product ID exists
    -- 4. Vendor is NOT Street Collector
...
-- 3. Initial Cleanup: Remove edition numbers from all Street Collector items
UPDATE "public"."order_line_items_v2"
SET edition_number = NULL,
    edition_total = NULL
WHERE LOWER(COALESCE(vendor_name, '')) IN ('street collector', 'street-collector')
AND edition_number IS NOT NULL;
