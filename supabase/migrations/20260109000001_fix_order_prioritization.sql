-- Migration: Update Collector View to correctly prioritize Shopify orders over #9 warehouse orders.
-- Orders starting with #9 or 9 are considered manual warehouse orders even if they don't have the WH- UUID prefix.
...
SELECT
  ...
  -- STRICT DEFINITION: Must have edition_number and belong to a non-duplicate order
  (
    SELECT COUNT(oli.id) 
