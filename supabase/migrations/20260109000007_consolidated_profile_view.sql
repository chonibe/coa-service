-- Migration: Final Consolidated Collector View
-- Ensures all orders associated with an email (via auth, orders, warehouse, or CRM) are grouped into a single profile.
...
unified_orders AS (
  -- Deduplicate orders by name, prioritizing Shopify (no WH- prefix) over Manual
...
SELECT
  ...
  -- Computed Display Fields (Order/Warehouse Name is often more current than Profile for guests)
  COALESCE(
    NULLIF(TRIM(cp.first_name || ' ' || cp.last_name), ''),
    (SELECT NULLIF(TRIM(o.customer_name), '') FROM orders o WHERE LOWER(o.customer_email) = cb.email AND o.customer_name IS NOT NULL AND o.customer_name != 'null' ORDER BY o.processed_at DESC LIMIT 1),
  ...
  COALESCE(
    ...
    (SELECT NULLIF(TRIM(o.customer_phone), '') FROM orders o WHERE LOWER(o.customer_email) = cb.email AND o.customer_phone IS NOT NULL ORDER BY o.processed_at DESC LIMIT 1),
  ...
  -- Statistics (ALL orders matching this email are summed here)
  (
    SELECT COUNT(oli.id) 
  ...
  -- This count correctly combines all orders matching this email from the unified pool
  ...
  -- PII Sources (JSON objects for detailed auditing)
  json_build_object(
    ...
    'shopify', (
      ...
      FROM orders o
      WHERE LOWER(o.customer_email) = cb.email
