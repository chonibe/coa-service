-- Migration: Fix Collector View to exclude empty or null emails from grouping.
-- This prevents anonymous orders from being aggregated into a single "ghost" profile.
...
SELECT
  ...
  COALESCE(
    ...
    (SELECT TRIM(concat_ws(' ', NULLIF(raw_shopify_order_data->'customer'->>'first_name', ''), NULLIF(raw_shopify_order_data->'customer'->>'last_name', ''))) FROM orders o WHERE LOWER(o.customer_email) = cb.email AND raw_shopify_order_data->'customer' IS NOT NULL ORDER BY o.processed_at DESC LIMIT 1),
    cb.email
  ) as display_name,
