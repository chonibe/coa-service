-- Update Payout Functions to handle multi-currency correctly
-- Only converts to USD if the order currency is NOT USD.
      ...
      SUM(
        CASE 
          WHEN ei.fulfillment_status = 'fulfilled' THEN
            CASE
              WHEN ei.created_at < '2025-10-01' THEN 10.00 -- Historical fix is always USD
              ELSE 
                -- Apply conversion ONLY if not USD
                (CASE 
                  WHEN UPPER(ei.currency) = 'GBP' THEN (COALESCE(ei.price, 0) * 1.27)
                  WHEN UPPER(ei.currency) IN ('NIS', 'ILS') THEN (COALESCE(ei.price, 0) * 0.27)
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN UPPER(COALESCE(o.currency_code, 'USD')) = 'GBP' THEN (COALESCE(oli.price, 0) * 1.27)
      WHEN UPPER(COALESCE(o.currency_code, 'USD')) IN ('NIS', 'ILS') THEN (COALESCE(oli.price, 0) * 0.27)
