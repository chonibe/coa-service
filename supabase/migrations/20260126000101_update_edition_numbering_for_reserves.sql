-- Migration: Update Edition Numbering for First Edition Reserves
-- Modifies assign_edition_numbers to start from #2 when first edition is reserved
-- Excludes internal reserve orders from edition counting
-- Date: 2026-01-26
...
DECLARE
    ...
    v_first_edition_reserved BOOLEAN := false;
BEGIN
    -- 1. Get the edition_size and first_edition_reserved flag from products
    SELECT 
        edition_size,
        COALESCE(first_edition_reserved, false) INTO v_edition_size_text, v_first_edition_reserved
    FROM products
    ...
    -- 2. If first edition is reserved, start numbering from 2
    IF v_first_edition_reserved THEN
        next_available_number := 2;
    END IF;

    -- 3. Preserve edition numbers for claimed/authenticated items
    -- EXCLUDING Street Collector (they should never have edition numbers)
    -- EXCLUDING items from invalid orders
    -- EXCLUDING internal reserve orders (source='internal_reserve')
    ...
    AND COALESCE(o.source, '') != 'internal_reserve';
    ...
    --    e) Items in internal reserve orders
    ...
    AND (
        ...
        OR COALESCE(o.source, '') = 'internal_reserve'
    );

    -- 5. Process all active non-Street Collector items in valid orders
    -- EXCLUDING internal reserve orders
    FOR line_item IN 
        SELECT li.id, li.order_id::TEXT as order_id_text, li.line_item_id, li.created_at, li.nfc_claimed_at, li.edition_number, li.owner_name, li.owner_email, li.shopify_customer_id, li.owner_id, li.vendor_name, o.order_name, o.source
        FROM "public"."order_line_items_v2" li
        JOIN "public"."orders" o ON o.id::TEXT = li.order_id::TEXT
        WHERE li.product_id::TEXT = p_product_id::TEXT
        AND li.status = 'active'
...
DECLARE
    v_order_source TEXT;
BEGIN
    -- Get order source to check if it's an internal reserve
    ...
    IF NEW.status = 'active' 
       ...
       AND LOWER(COALESCE(NEW.vendor_name, '')) NOT IN ('street collector', 'street-collector')
       AND COALESCE(v_order_source, '') != 'internal_reserve' THEN
