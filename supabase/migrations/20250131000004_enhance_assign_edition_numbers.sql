-- Migration: Enhance assign_edition_numbers Function
-- Updates function to preserve authenticated editions and handle uniqueness constraints
...
BEGIN
    ...
    FOR line_item IN 
        SELECT id, line_item_id, created_at, nfc_claimed_at, edition_number, owner_name, owner_email, fulfillment_status, status
        FROM "public"."order_line_items_v2"
        WHERE "product_id"::text = p_product_id::text
        AND status = 'active'
