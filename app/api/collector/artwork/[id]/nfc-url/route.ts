import type { NextRequest } from "next/server"
import { NextResponse } from "next/server"
import { cookies } from "next/headers"
import { createClient } from "@/lib/supabase/server"
import { signPayload } from "@/lib/nfc/token"

/**
 * Return the canonical URL a collector should write to their own NFC tag
 * for a given artwork (line item).
 *
 * The URL is shaped exactly like admin-signed programming URLs:
 *   `{origin}/api/nfc-tags/redirect?tagId=<lineItemId>&token=<signedToken>`
 *
 * so both paths flow through the same resolver and land on
 * `/collector/artwork/[lineItemId]`.
 */

function getShopifyCustomerId(cookieStore: any): string | null {
  const shopifyCustomerId = cookieStore.get("shopify_customer_id")
  return shopifyCustomerId?.value || null
}

export async function GET(
  request: NextRequest,
  { params }: { params: { id: string } },
) {
  try {
    const cookieStore = cookies()
    const shopifyCustomerId = getShopifyCustomerId(cookieStore)

    if (!shopifyCustomerId) {
      return NextResponse.json({ error: "Not authenticated" }, { status: 401 })
    }

    const supabase = createClient()
    const lineItemId = params.id

    const { data: lineItem, error: lineItemError } = await supabase
      .from("order_line_items_v2")
      .select(
        `
          id,
          line_item_id,
          order_id,
          edition_number,
          orders:order_id ( shopify_customer_id )
        `,
      )
      .eq("line_item_id", lineItemId)
      .single()

    if (lineItemError || !lineItem) {
      return NextResponse.json({ error: "Artwork not found" }, { status: 404 })
    }

    const order = lineItem.orders as any
    if (!order || order.shopify_customer_id !== shopifyCustomerId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 403 })
    }

    // The `tagId` written to the physical tag is the line item id itself.
    // This lets the canonical `/api/nfc-tags/redirect` resolve straight to
    // the artwork via `nfc_tags.tag_id` OR via the signed token payload.
    const tagId = lineItem.line_item_id

    const expiresAt = Date.now() + 60 * 60 * 24 * 365 * 1000 // 1 year
    const token = signPayload({
      tagId,
      lineItemId: lineItem.line_item_id,
      orderId: lineItem.order_id,
      editionNumber: lineItem.edition_number,
      exp: expiresAt,
    })

    const baseUrl =
      process.env.NEXT_PUBLIC_APP_URL ||
      request.nextUrl.origin ||
      "https://app.thestreetcollector.com"

    const nfcUrl = `${baseUrl}/api/nfc-tags/redirect?tagId=${encodeURIComponent(
      tagId,
    )}&token=${encodeURIComponent(token)}`

    return NextResponse.json({
      success: true,
      nfcUrl,
      tagId,
      expiresAt,
      instructions: [
        "1. Download an NFC writing app (NFC Tools, TagWriter, etc.)",
        "2. Create a new 'URL' record",
        "3. Paste this URL",
        "4. Write to your NTAG215 or similar tag",
        "5. Tap your new tag to authenticate",
      ],
    })
  } catch (error: any) {
    console.error("Error generating NFC URL:", error)
    return NextResponse.json(
      { error: "Internal server error", message: error.message },
      { status: 500 },
    )
  }
}
