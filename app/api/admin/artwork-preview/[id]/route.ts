 ...
 * Allows admin users to view collector artwork experience without requiring collector authentication
 */
export async function GET(
  ...
  try {
    ...
    // For now, this endpoint is accessible to anyone with the link
    // In production, verify the user has admin privileges
    ...
    if (lineItemError || !lineItem) {
      console.error(`[Admin Artwork Preview API] Line item not found: ${lineItemId}`, lineItemError)
    ...
    // Build content blocks - always return them for viewing
    ...
    if (seriesMember && series && order?.shopify_customer_id) {
      ...
      const { data: ownedLineItems } = await supabase
        .from("order_line_items_v2")
        .select("product_id, orders!inner(shopify_customer_id)")
        .in("product_id", seriesArtworks?.map((a: any) => a.shopify_product_id) || [])
        .eq("orders.shopify_customer_id", order.shopify_customer_id)
    ...
    return NextResponse.json({
      ...
      isAuthenticated,
      canInteract: false, // Admin preview mode - no interactions allowed
      specialChips,
      discoveryData,
      isAdminPreview: true, // Flag to indicate this is admin preview mode
    })
  } catch (error: any) {
    console.error("Error in admin artwork preview API:", error)
    return NextResponse.json(
