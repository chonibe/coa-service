...
export async function fetchInstagramMedia(artistId: string, limit = 6) {
  try {
    ...
    try {
      ...
      // Cache miss or table doesn't exist, continue to fetch from API
      console.log("Cache error (expected if tables don't exist yet):", error)
    }
    ...
    if (!credentials || !credentials.instagram_business_id || !credentials.access_token) {
      console.error("Missing Instagram credentials:", credentials)
      return { error: "Invalid Instagram credentials" }
    }

    console.log("Using Instagram Business ID:", credentials.instagram_business_id.substring(0, 5) + "...")
