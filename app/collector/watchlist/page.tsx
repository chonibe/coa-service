...
const watchlistContent = getCollectorPageContent('watchlist')
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-[#141010] px-4 py-8 max-w-lg mx-auto font-body">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-neutral-900 dark:text-[#f4f0f0] font-body">{watchlistContent.hero.title}</h1>
          <p className="text-sm text-neutral-600 dark:text-[#b0a0a0]">
            {loading ? watchlistContent.hero.loadingCounts : watchlistContent.hero.counts(counts.total, counts.available)}
          </p>
        </div>
      </div>

      <p className="text-sm text-neutral-600 dark:text-[#a89898] mb-6 leading-relaxed">
        {watchlistContent.hero.intro}
      </p>

      {loading ? (
        <p className="text-sm text-neutral-500 dark:text-[#908080]">{watchlistContent.loading}</p>
      ) : items.length === 0 ? (
              ...
              >
                <div className="flex justify-between gap-3 items-start">
                  <div className="min-w-0">
                    ...
                    >
                      {row.product_title || watchlistContent.item.untitled}
                    </Link>
                    {row.artist_name && (
                    ...
                    <p className="text-[10px] uppercase tracking-[0.08em] text-neutral-500 dark:text-[#806868] mt-2">
                      {watchlistContent.item.stageWhenSaved(row.stage_at_save)}
                    </p>
                  </div>
                  <Button
                    type="button"
                  ...
                  >
                    {removing === row.shopify_product_id ? watchlistContent.item.removing : watchlistContent.item.remove}
                  </Button>
                </div>
              </li>
            )
