...
export default async function StreetCollectorPage() {
  ...
  return (
    <div className="w-full bg-background text-foreground pb-16 md:pb-0">
      <StreetCollectorBrandJsonLd />
      {/* Thin promo bar — shipping / guarantee / returns (above nav on desktop, top of page on mobile) */}
      <div className="fixed top-0 left-0 right-0 z-[122] hidden shadow-none [filter:none] md:flex flex-col">
        <div
          className="flex w-full items-center justify-center border-b border-border bg-background px-3 py-1 text-center text-[11px] font-medium leading-tight tracking-wide text-muted-foreground shadow-none sm:text-xs sm:py-1.5"
      ...
      <div
        className="fixed top-0 left-0 right-0 z-[122] border-b border-border bg-background py-1 shadow-none [filter:none] md:hidden"
        style={{ paddingTop: 'max(0.25rem, env(safe-area-inset-top, 0px))' }}
        role="region"
        aria-label="Shipping, guarantee, and returns"
      >
        <p className="px-2 text-center text-[10px] font-medium leading-snug tracking-wide text-muted-foreground sm:text-[11px]">
          {trustPromoLine}
        </p>
      </div>
      ...
      >
        <Link
          href={streetCollectorContent.experienceUrl}
          prefetch={false}
          className={streetCollectorCtaStickyClass}
        >
        ...
        <SectionWrapper spacing="xs" background="experience" className="!py-8 sm:!py-10">
          <Container maxWidth="default" paddingX="gutter">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="font-serif text-2xl font-medium tracking-tight text-experience-title sm:text-3xl md:text-4xl">
                {streetCollectorContent.funnelBridge.title}
              </h2>
              <p className="mt-2 text-sm text-muted-foreground sm:text-base">
                {streetCollectorContent.funnelBridge.subtitle}
              </p>
              <ol className="mt-6 space-y-3 text-left text-sm text-foreground/90 sm:text-base">
                {streetCollectorContent.funnelBridge.steps.map((step, i) => (
                  <li key={step} className="flex gap-3">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-experience-cta/15 text-xs font-bold text-experience-cta">
                      {i + 1}
                    </span>
              ...
              <p className="mt-4 text-xs text-muted-foreground sm:text-sm">{streetCollectorContent.funnelBridge.reassurance}</p>
      ...
      {featuredArtists.length > 0 && (
        <ArtistCarousel
          ...
          titleClassName="font-serif font-medium text-experience-title"
          sectionBackground="experience"
          arrowButtonClassName="bg-experience-highlight text-background"
          ...
          trailingContent={
            streetCollectorContent.featuredArtists.afterCarousel ? (
              <p className="text-center font-body text-lg text-muted-foreground sm:text-xl md:text-2xl">
                {streetCollectorContent.featuredArtists.afterCarousel}
              </p>
            ) : null
          }
          leadingContent={
            <div className="space-y-10 sm:space-y-6">
              <h2 className="font-body font-medium text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-experience-title tracking-tight text-center">
      ...
      >
        <Container maxWidth="default" paddingX="gutter">
          {streetCollectorContent.trustBarTitle ? (
            <h2 className="mb-10 text-center font-serif text-3xl font-medium tracking-tight text-experience-title sm:mb-12 sm:text-4xl md:mb-14 md:text-5xl">
              {streetCollectorContent.trustBarTitle}
            </h2>
          ) : null}
          ...
          <div className="mx-auto flex max-w-md flex-col items-center gap-10 md:hidden">
            {streetCollectorContent.trustBar.map((item) => (
              ...
              >
                ...
                <p className="max-w-[22rem] px-1 text-sm font-bold leading-snug text-experience-title">
                  {item.label}
                </p>
              </div>
            ))}
          ...
          <div className="hidden py-2 md:grid md:grid-cols-3 md:items-stretch md:gap-6 lg:gap-8 md:py-8">
            {streetCollectorContent.trustBar.map((item) => (
              <div
                key={item.label}
                className="flex h-full w-full flex-col rounded-2xl border border-border bg-card/80 p-6 text-center shadow-sm lg:p-8"
              >
                <div className="flex h-28 shrink-0 items-center justify-center lg:h-32">
                  <TrustBarItemIcon item={item} variant="featured" />
                </div>
                <p className="text-base font-bold leading-snug text-experience-title lg:text-lg">
                  {item.label}
                </p>
                <p className="mt-2 min-h-0 flex-1 text-base font-normal leading-relaxed text-muted-foreground">
                  {item.description ?? ''}
                </p>
              </div>
            ))}
