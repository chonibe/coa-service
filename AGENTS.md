# Working with this repo in Codex
...
| `artifacts/Codex-skills/*.skill` | Codex-style zip bundles, produced by `npm run skills:pack` (folder is gitignored). |

### What you should do

1. **Read or change skills** under **`skills/`**, not by assuming `.cursor/skills/` is authoritative (sync can overwrite it).
2. **To produce installable `.skill` files for Codex** (Desktop, Cowork-style, etc.), from repo root run:
   - `npm run skills:pack` — all skills  
   - `npm run skills:pack -- <skill-name>` — one skill  
   Then use the files under **`artifacts/Codex-skills/`**.
...
## Getting **Codex** to use this (not only Cursor)

**1. Same repo open in Codex (or any tool that reads the project)**  
Treat **`skills/`** as canonical: read and edit files there. This file (**`AGENTS.md`**) plus **`skills/README.md`** tell you the workflow. After hub **`skills:org:pull`**, run **`npm run skills:pack`** if you need **`.skill`** files for another Codex app.

**2. Codex Desktop / mobile / web Projects**  
- Add **project knowledge**: upload **`AGENTS.md`** (or **`skills/README.md`**).  
- For **Skills** (or “custom skills” / `.skill` installs, depending on product): run **`npm run skills:pack`** locally, then install each **`artifacts/Codex-skills/<name>.skill`** the way your Codex client expects. Re-pack after **`skills/`** changes.
...
## Getting **Codex** to **evolve** skills too
...
**A — Interactive (Codex in the loop)**  
...
3. Run **`npm run skills:pack`** (for Codex bundles) and/or **`npm run skills:org:push-proposal`** (for hub review). Cursor users run **`npm run skills:sync`** after you change **`skills/`**.
...
**C — Org-wide**  
After Codex (or anyone) improves **`skills/`**, merge via git and/or **`skills:org:publish`** on the hub so other teams **`pull`** the same version.
