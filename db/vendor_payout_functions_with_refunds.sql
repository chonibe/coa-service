...
BEGIN
  ...
  refund_deductions AS (
    ...
    -- This represents money that needs to be deducted from future payouts
    SELECT 
      oli.vendor_name,
      SUM(
        CASE 
  ...
  -- Include all vendors, even if they have $0 or negative pending
  ORDER BY avt.amount DESC;
END;
$$ LANGUAGE plpgsql;

-- Function to get line items for a specific vendor that haven't been paid yet
-- Only returns fulfilled line items (excluding refunded items)
...
-- Function to get vendor balance including refund deductions
-- Returns the net balance (pending payouts minus refund deductions)
CREATE OR REPLACE FUNCTION get_vendor_balance(p_vendor_name TEXT)
RETURNS TABLE (
...
BEGIN
  ...
  refund_items AS (
    SELECT 
      ...
      COALESCE(pvp.payout_amount, 25) as payout_amount,
      COALESCE(pvp.is_percentage, true) as is_percentage,
      vpi.amount as paid_amount
    FROM order_line_items_v2 oli
    LEFT JOIN product_vendor_payouts pvp ON oli.product_id::TEXT = pvp.product_id::TEXT AND oli.vendor_name = pvp.vendor_name
    INNER JOIN vendor_payout_items vpi ON oli.line_item_id::TEXT = vpi.line_item_id::TEXT AND vpi.payout_id IS NOT NULL
    WHERE 
      oli.vendor_name = p_vendor_name
      AND oli.fulfillment_status = 'fulfilled'
      AND oli.refund_status IN ('partial', 'full')
  ...
  SELECT 
    ...
    COALESCE(
      SUM(
        CASE 
          WHEN ri.refund_status = 'full' THEN COALESCE(ri.paid_amount, 0)
          WHEN ri.refund_status = 'partial' THEN 
            COALESCE(ri.refunded_amount, 0) * 
            CASE 
              WHEN ri.is_percentage THEN (ri.payout_amount / 100)
              ELSE (ri.payout_amount / NULLIF(ri.price, 0))
            END
          ELSE 0
        END
      ),
  ...
  FROM pending_items pi
  FULL OUTER JOIN refund_items ri ON pi.vendor_name = ri.vendor_name
  WHERE (pi.vendor_name = p_vendor_name OR ri.vendor_name = p_vendor_name);
END;
$$ LANGUAGE plpgsql;
