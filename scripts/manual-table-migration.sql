-- Manual Migration Script for order_line_items_v2 to order_line_items

-- Backup the original table (optional but recommended)
CREATE TABLE order_line_items_backup AS 
...
-- Verification query
SELECT COUNT(*) AS total_records FROM order_line_items;
SELECT COUNT(*) AS backup_records FROM order_line_items_backup;

-- Optional: Drop backup table after verification
-- DROP TABLE order_line_items_backup; 