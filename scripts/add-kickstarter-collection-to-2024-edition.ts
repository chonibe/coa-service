...
async function addKickstarterCollectionTo2024Edition() {
  try {
    ...
    // Now find the "2024 Edition" series for each vendor
    console.log('\n🔍 Finding "2024 Edition" series...\n')
    ...
    for (const [vendorName, vendorProducts] of productsByVendor.entries()) {
      try {
        ...
        // Find the "2024 Edition" series for this vendor
        ...
        if (!series) {
          console.log(`   ⚠️  "2024 Edition" series not found for vendor ${vendorName}`)
          totalNotFound += vendorProducts.length
          results.push({
            vendorName,
            vendorId: vendor.id,
...
addKickstarterCollectionTo2024Edition()
