...
async function updateViewFinal() {
  const VIEW_SQL = `
...
-- Deduplicate orders by numeric name prefix, prioritizing Shopify (numeric/gid) over Manual (WH-)
unified_orders AS (
  SELECT DISTINCT ON (COALESCE(SUBSTRING(order_name FROM '^[#]*(\\d+)'), id)) 
    *
  FROM orders
  ORDER BY COALESCE(SUBSTRING(order_name FROM '^[#]*(\\d+)'), id), 
    CASE 
...
SELECT
  ...
  -- Statistics (counting from unified_orders only)
  (
    SELECT COUNT(oli.id) 
    ...
    WHERE 
      (
        LOWER(uo.customer_email) = cb.email
        OR uo.customer_id = (
          SELECT raw_shopify_order_data->'customer'->>'id' 
          FROM orders 
          WHERE LOWER(customer_email) = cb.email 
          AND raw_shopify_order_data->'customer'->>'id' IS NOT NULL 
          LIMIT 1
        )
      ) 
      AND oli.status = 'active'
      AND oli.edition_number IS NOT NULL
      AND uo.fulfillment_status NOT IN ('restocked', 'canceled')
      AND uo.financial_status NOT IN ('refunded', 'voided')
  ) as total_editions,
  ...
  (
    SELECT COALESCE(SUM(uo.total_price), 0) 
    FROM unified_orders uo 
    WHERE 
      (
        LOWER(uo.customer_email) = cb.email
...
LEFT JOIN collector_profiles cp ON cp.user_id = u.id;
  ...
  const { error } = await supabase.rpc('exec_sql', { sql_query: VIEW_SQL });
  if (error) console.error('Error:', error);
  else console.log('✅ Successfully updated collector_profile_comprehensive view with Shopify-first precedence.');
}
