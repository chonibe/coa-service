-- RBAC System: Complete Migration Script (Fixed for Permission Issues)
-- Run this script directly in Supabase Dashboard SQL Editor
-- This combines all 4 RBAC migrations into one script with proper permission handling
-- Date: 2026-01-26
...
-- ============================================
-- PART 2: JWT Hook Functions (Public Schema Only)
-- ============================================
...
-- Helper function for JWT role checking (in public schema)
...
-- Helper function for JWT permission checking (in public schema)
...
-- Helper to get vendor_id from JWT (in public schema)
...
-- Helper to check if user is admin (in public schema)
...
-- ============================================
-- PART 4: Update RLS Policies to use public.has_role
-- ============================================

-- Note: Using public.has_role() instead of auth.has_role() to avoid permission issues
-- The functionality is identical, just using the public schema function
...
DROP POLICY IF EXISTS "Admins can manage all messages" ON public.vendor_messages;
...
DROP POLICY IF EXISTS "Admins can manage all notifications" ON public.vendor_notifications;
...
DROP POLICY IF EXISTS "Admins can manage all items" ON public.order_line_items_v2;
...
DROP POLICY IF EXISTS "Collectors can view their own profile" ON public.collector_profiles;
DROP POLICY IF EXISTS "Collectors can update their own profile" ON public.collector_profiles;
DROP POLICY IF EXISTS "Collectors can insert their own profile" ON public.collector_profiles;
DROP POLICY IF EXISTS "Admins can view all profiles" ON public.collector_profiles;
DROP POLICY IF EXISTS "Admins can manage all profiles" ON public.collector_profiles;
...
CREATE POLICY "Public can view all products"
  ON public.products FOR SELECT
  USING (true);  -- Products table doesn't have status column, allow public viewing
...
BEGIN
  ...
  RAISE NOTICE '1. JWT hook is already configured in config.toml';
  RAISE NOTICE '2. Test login and verify JWT contains user_roles claim';
  RAISE NOTICE '3. Update API routes to use new withAuth middleware';
  RAISE NOTICE '4. RLS policies now use public.has_role() functions';
  RAISE NOTICE '================================';
END $$;
