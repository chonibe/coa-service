...
- **Change Log**:
  - 2026-04-06: **Shop discounts admin** – `/admin/shop/discounts` lists lamp volume toggle and **featured artist bundle** settings (on/off, mode: fixed total / % off / $ off, value); `GET`/`PATCH` `/api/admin/shop/discount-flags` persists `lampArtworkVolume`, `featuredBundleEnabled`, `featuredBundleMode`, `featuredBundleValue` under `system_settings.shop_discount_flags`. Shop experience loads full settings with `getShopDiscountSettings()` and `ShopDiscountFlagsProvider` ([`ShopDiscountFlagsContext.tsx`](../../../app/(store)/shop/experience-v2/components/ShopDiscountFlagsContext.tsx)); bundle pricing stays on when extra artworks are added ([`lib/shop/experience-bundle-order-pricing.ts`](../../../lib/shop/experience-bundle-order-pricing.ts)).
  - 2026-04-06: **Polaris Switch visibility** – [`PolarisSwitch`](../../../components/polaris/polaris-switch.tsx) uses **`peer` / `peer-checked`** for the checked track color and thumb slide (more reliable than `has-[:checked]` on the label). Shop discounts page shows **On/Off** next to each toggle and disables only the row being saved (not every switch).
  - 2025-12-11: **UX Refresh** – grouped navigation, command palette, overview-first admin home, and vendor explorer filter improvements.
    ...
    - Added skip-link and labeled navigation for accessibility across admin shell surfaces (`app/admin/admin-shell.tsx`).
