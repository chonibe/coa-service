...
- **Person profiles (cohorts):** Init uses `person_profiles: "always"` so anonymous visitors get person records. With `identified_only`, `setPersonProperties` for A/B variant, device, and quiz fields did not reliably populate cohort person filters.
...
- **User properties:** `setUserProperty(key, value)` from [`lib/posthog.ts`](../../lib/posthog.ts) sets person properties. Properties tracked: `preferred_device`, `is_returning_user`, `experience_ab_variant`, `quiz_owns_lamp`, `quiz_purpose` (on experience quiz completion), `total_purchases`, `first_purchase_at` (order tracking page). On login, `identify()` merges quiz + A/B from storage via `getPostHogIdentifyTraitsFromClientStorage()`.
