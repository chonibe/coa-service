...
9. **Wheel over 3D column → artwork scroll (2026-03-20)** — On the Configurator, the 3D preview and artwork grid are **siblings** inside a fixed layout: the pointer may sit over the model while the only vertical scroll container is the grid/detail panel. Native wheel does not scroll a sibling. The Spline runtime also attaches `wheel` on the **canvas** (often stopping propagation), so a listener on the preview **container** never runs when the cursor is over the model. `Spline3DPreview` (minimal mode) uses **`document` capture** (`passive: false`), hit-tests the preview with `getBoundingClientRect()`, then scrolls the nearest scrollable **ancestor** of the preview root (e.g. `SplineFullScreen` reel) or the first `[data-experience-artwork-scroll]` region, and calls `preventDefault` + `stopPropagation` so WebGL does not eat the gesture.
