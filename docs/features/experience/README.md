...
The Experience page (`/shop/experience`) lets users customize a Street Lamp with artwork. It includes an intro quiz, a 3D Spline preview, an artwork strip, filters, and checkout. The **collection carousel strip** ([`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx)) sits **above** the main Spline reel in normal layout (same as v2). A sticky bottom bar shows **only** **“Choose your first artwork”** when the collection is empty (no checkout on that bar — use the header cart). Once art is added, the bar shows thumbnails, **Checkout** (same OrderBar as the header cart), and a **purple/violet** add FAB. On **mobile** (viewport under `md`), the **artwork carousel strip is hidden**; **cart thumbnails** on the sticky checkout bar are the only strip, and **tapping a thumbnail** selects that item on the **Spline** preview. See [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx). **Featured artist bundle ($159)** (spotlight + two prints, allocated Stripe lines) is documented in [Experience V2 README](../experience-v2/README.md#featured-artist-bundle-159) and implemented in [`ExperienceV2Client`](../../../app/(store)/shop/experience/components/ExperienceV2Client.tsx) (this route) and the v2 shell.
