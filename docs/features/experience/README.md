...
13. **Gallery sync without fighting scroll (2026-04-09)** — [`ArtworkInfoBar`](../../../app/(store)/shop/experience/components/ArtworkInfoBar.tsx) calls `onGalleryImagesChange` whenever its `galleryImages` memo updates (including when the full product loads from `/api/shop/products/[handle]`). [`ExperienceV2Client`](../../../app/(store)/shop/experience/components/ExperienceV2Client.tsx) (and the v2 shell) now compare a **content signature** ([`reelGalleryItemsSignature`](../../../lib/shop/experience-reel-gallery.ts)) and only call `bumpReelAlign()` / reset slide when the gallery **materially** changes; **strict tail extensions** (same ordered prefix, more images only at the end) update state without `scrollIntoView`. The slide clamp `useEffect` no longer lists `previewSlideIndex` as a dependency so it does not re-run on every scroll-driven index update.
...
- Version: 1.15.15
- **Reviews + welcome incentive (home)**: Live Yotpo store review summary powers hero/testimonials (“5.0 from N reviews”) via [`yotpo-store-reviews`](../../../lib/shop/yotpo-store-reviews.ts). First-visit welcome strip sits **in-flow below TrustBar** on home-v2 when enabled; Final CTA can still reveal the code after newsletter signup. **Off by default** — set `NEXT_PUBLIC_WELCOME_INCENTIVE_ENABLED=1`. Config: [`welcome-incentive`](../../../lib/shop/welcome-incentive.ts) / [`WelcomeIncentiveStrip`](../../../components/shop/WelcomeIncentiveStrip.tsx) (Stripe Promotion Code; see `.env.example`).
- **Last-viewed artwork + random preview**: When visitors open `/shop/experience` (or `/experience`) **without** `?artwork=` **and without** a saved last-viewed artwork, [`pickInitialPreviewProduct`](../../../lib/shop/experience-gallery-images.ts) chooses a **uniform random** product from the in-season (season 2) pool when non-empty, else season 1 — preferring `availableForSale`. The server picks once per request so hero preload and [`initialGalleryProduct`](../../../app/(store)/shop/experience/page.tsx) match the first paint. Explicit `?artwork=` deep links still win. Client restore: [`experience-last-viewed-artwork`](../../../lib/shop/experience-last-viewed-artwork.ts) persists the preview in `localStorage` and [`ExperienceV3Client`](../../../app/(store)/shop/experience-v3/components/ExperienceV3Client.tsx) restores it on return (priority: deep link → last-viewed → random). Cart / lamp-preview persistence still does **not** pin the preview (only cart contents).
- **Gallery hover second image**: Experience artwork grid cards and home best-sellers gallery swap to the product’s second image on desktop hover (`@media (hover: hover)`); on touch devices, press/active peeks the second image ([`ExperienceArtworkGridCard`](../../../app/(store)/shop/experience/components/ExperienceArtworkGridCard.tsx)).
- **Lamp-only cart badge (mobile)**: “Lamp added to your collection” pill beside the cart chip is **desktop-only** (`hidden md:inline-flex`) so it no longer clips the toolbar on small screens ([`ExperienceSlideoutMenu`](../../../app/(store)/shop/experience-v2/ExperienceSlideoutMenu.tsx)).
- **Two-sided lamp cart hint**: When the cart has a lamp and exactly one artwork, [`OrderBar`](../../../app/(store)/shop/experience-v2/components/OrderBar.tsx) shows an inline hint and [`ExperienceCartChip`](../../../app/(store)/shop/experience-v2/ExperienceCartChip.tsx) surfaces the same nudge via `title` tooltip.
- **Lamp dimensions**: Specs / materials copy use **21.5×14.5×7 cm** (~8.5×5.7×2.8 in) across FAQ, Experience product info, and lamp detail specs.
- **Stripe “Default Title”**: Checkout line-item descriptions strip Shopify’s `Default Title` via [`stripeLineItemDescription`](../../../lib/shop/stripe-line-item-description.ts) (OrderBar + `/api/checkout/stripe` + `/api/checkout/create`).
- **Purchase clarity (scarcity / ladder / hold)**: Inline “What this means” helpers and always-visible hold microcopy explain limited runs, street ladder price steps, and edition holds. Copy lives in [`lib/shop/experience-purchase-hints.ts`](../../../lib/shop/experience-purchase-hints.ts); UI via [`ExperienceMeaningHint`](../../../app/(store)/shop/experience-v2/components/ExperienceMeaningHint.tsx). Street ladder is wired into [`ExperienceV3EditionStrip`](../../../app/(store)/shop/experience-v3/components/ExperienceV3ProductPanel.tsx) (was previously dropped). Surfaces: edition strip, [`StreetLadderScarcityAddon`](../../../app/(store)/shop/experience-v2/components/StreetLadderScarcityAddon.tsx), [`EditionHoldIndicator`](../../../app/(store)/shop/experience-v2/components/EditionHoldIndicator.tsx) banner, sticky [`ExperienceV3FloatingAddCard`](../../../app/(store)/shop/experience-v3/components/ExperienceV3FloatingAddCard.tsx).
- **Shipping timeline before Stripe**: Trust strip / home / OrderBar / slideout now show **Free worldwide shipping · 9–15 business days** (aligned with [`content/shop-faq.ts`](../../../content/shop-faq.ts) and [`content/experience-v3-product-info.ts`](../../../content/experience-v3-product-info.ts)). Stripe Checkout delivery estimate updated to 9–15 business days in [`lib/shop/stripe-checkout-shipping.ts`](../../../lib/shop/stripe-checkout-shipping.ts).
- **Stripe Checkout prefers card (no Link-first)**: Hosted Checkout sessions omit `link` from `payment_method_types` (`['card', 'paypal']`) so shoppers land on the card form; Apple Pay / Google Pay still available via `card`. Payment Element sets `wallets.link: 'never'` and orders wallets/card ahead of PayPal. Routes: [`app/api/checkout/create`](../../../app/api/checkout/create/route.ts), [`stripe`](../../../app/api/checkout/stripe/route.ts), [`create-checkout-session`](../../../app/api/checkout/create-checkout-session/route.ts); Elements: [`PaymentStep`](../../../components/shop/checkout/PaymentStep.tsx), [`CardInputSection`](../../../components/shop/checkout/CardInputSection.tsx).
- **Mobile collection picker close + swipe dismiss**: [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx) modal sheet now sits **below** the fixed [`ShopUnifiedTopBar`](../../../components/shop/navigation/ShopUnifiedTopBar.tsx) (`z-[122]`) using [`shopUnifiedTopBarTopInsetClass`](../../../lib/shop/shop-unified-top-bar-layout.ts) / drawer height helpers, so **Done** is no longer covered by the top bar. Mobile bottom sheet supports **swipe-down-to-dismiss** from the header drag handle (framer-motion `dragControls`, same thresholds as StoryViewer). Top-bar / under-CTA Collection toggles remain usable.
- **Mobile under-CTA Collection button**: On mobile (below `lg`), [`ExperienceV3StickyAddPanel`](../../../app/(store)/shop/experience-v3/components/ExperienceV3StickyAddPanel.tsx) shows **The Collection** beneath the floating add CTA; click uses the same `handleCollectionButtonClick` / picker toggle as the top-bar pill ([`ExperienceV3Client`](../../../app/(store)/shop/experience-v3/components/ExperienceV3Client.tsx)). Hidden on desktop rail layouts.
- **Mobile experience scroll (fixed shell)**: Document scroll is intentionally disabled (`fixed inset-0` + `h-dvh` + `overflow-hidden` in [`experience/layout.tsx`](../../../app/(store)/shop/experience/layout.tsx) / [`experience-v3/layout.tsx`](../../../app/(store)/shop/experience-v3/layout.tsx)). The children wrapper uses an **absolute inset fill** so the main column (`data-experience-scroll-root` in [`ExperienceV3Client`](../../../app/(store)/shop/experience-v3/components/ExperienceV3Client.tsx)) stays a reliable `overflow-y-auto` scrollport on iOS after `html`/`body` switched to `min-height: 100%` (see [`globals.css`](../../../app/globals.css)). Horizontal strips use `[touch-action:pan-x_pan-y]` so vertical swipes are not trapped by `touch-pan-x`.
- **Mobile Collection attention nudge**: On mobile, the top-bar **The Collection** pill ([`ExperienceV3Client`](../../../app/(store)/shop/experience-v3/components/ExperienceV3Client.tsx)) softly pulses/wiggles (`.animate-collection-btn-attention` in [`globals.css`](../../../app/globals.css)) until the first tap; dismissal is stored in `localStorage` (`experience-v3-collection-button-clicked`). Respects global `prefers-reduced-motion`.
...
- **Watch + edition copy under scarcity (reel)**: [`ArtworkAccordions`](../../../app/(store)/shop/experience/components/ArtworkAccordions.tsx) no longer renders a separate [`ArtworkEditionUnifiedSection`](../../../app/(store)/shop/experience-v2/components/ArtworkEditionUnifiedSection.tsx) / [`EditionBadgeForProduct`](../../../app/(store)/shop/experience-v2/components/EditionBadge.tsx) block (including **`editionOnly`** above Spline — that slot is empty). [`EditionWatchWithNarrative`](../../../app/(store)/shop/experience-v2/components/EditionWatchWithNarrative.tsx) sits under the Street ladder when present, or below the scarcity bar when there is no ladder; it includes **`Watch this edition`** plus small subline/CTA text from [`edition-stages`](../../../lib/shop/edition-stages.ts).
- **Checkout sticky bar (thumbnails)**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) shows **lamp tile(s) only when `lampQuantity > 0`**, then artwork thumbnails, with **`+`** separators (same strip on mobile and desktop; no separate “N artworks” text row).
- **Title opens detail + reel sync**: [`ExperienceV2Client`](../../../app/(store)/shop/experience/components/ExperienceV2Client.tsx) **`handleViewDetail`** scrolls the vertical reel to the **details** section when opening the information panel (slide **1** for lamp, slide **2** for artworks with edition-before-Spline layout), matching [`ArtworkInfoBar`](../../../app/(store)/shop/experience/components/ArtworkInfoBar.tsx) **`detailSlide`** indices.
