...
9. **Wheel over 3D column → artwork scroll (2026-03-20)** — On the Configurator, the 3D preview and artwork grid are **siblings** inside a fixed layout: native wheel does not scroll a sibling. The WebGL **canvas** often still receives `wheel` before parents even with `pointer-events: none`, so `Spline3DPreview` (minimal mode) adds a **transparent full-size layer** (`z-[5]`) **above** the canvas with `{ passive: false }` `wheel` handling, scrolls the nearest scrollable **ancestor** of the preview root (e.g. `SplineFullScreen` reel) or the first visible `[data-experience-artwork-scroll]` region, then `preventDefault` + `stopPropagation`. Loading overlay stays `pointer-events: none` so wheel reaches this layer while the spinner shows.
