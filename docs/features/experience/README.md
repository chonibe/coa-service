...
- **Checkout sticky bar (mobile)**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) shows **[lamp] + `N artwork` / `N artworks`** (no “added” suffix; the **`+`** remains the separate separator between lamp and copy).
