...
The Experience page (`/shop/experience`) lets users customize a Street Lamp with artwork. It includes an intro quiz, a 3D Spline preview, an artwork strip, filters, and checkout. On **mobile**, the **collection carousel strip** sits **above** the main Spline reel (under the experience chrome); **desktop** keeps the strip as a **bottom overlay** (same as v2 — shared [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) with **`dockCarouselTopOnMobile`**). A sticky bottom bar shows **only** **“Choose your first artwork”** when the collection is empty (no checkout on that bar — use the header cart). Once art is added, the bar shows thumbnails, **Checkout** (same OrderBar as the header cart), and a **centered overlay** add control. See [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx). **Featured artist bundle ($159)** (spotlight + two prints, allocated Stripe lines) is documented in [Experience V2 README](../experience-v2/README.md#featured-artist-bundle-159) and implemented in [`ExperienceV2Client`](../../../app/(store)/shop/experience/components/ExperienceV2Client.tsx) (this route) and the v2 shell.
