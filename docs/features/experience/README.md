...
9. **Wheel: `parentScrollMode` `contain` vs `isolate` (2026-03-20+)** — `Spline3DPreview` (minimal) uses **`parentScrollMode`**: **`contain`** ([`SplineFullScreen`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx)) means the preview sits inside a parent vertical scroller — **no** wheel-capture overlay and **no** JS `preventDefault`; native wheel/touch scrolls the reel. **`isolate`** (default; [Configurator](../../../app/(store)/shop/experience-v2/components/Configurator.tsx), template preview) means the lamp column and artwork list are **siblings** — a transparent layer above the canvas captures `wheel`, forwards to the first scrollable ancestor or visible `[data-experience-artwork-scroll]`, then `preventDefault` + `stopPropagation`. Loading overlay stays `pointer-events: none` so wheel reaches the layer while the spinner shows (isolate only).
10. **Reel: suppress slide sync during `scrollIntoView`** — When thumbnails call `onGoToSlide`, `SplineFullScreen` runs smooth `scrollIntoView`. Mid-animation, midpoint-based `handleScroll` could report the wrong section and fight the parent `currentSlide`. `ignoreSlideSyncUntilRef` blocks `onSlideChange` from `handleScroll` until `scrollend` on the reel (where supported) or an ~850ms timeout; then `lastReportedSectionRef` is aligned to the target slide.
