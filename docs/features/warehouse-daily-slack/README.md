...
1. **Open orders** (not canceled, not delivered): **recipient name** (`ship_name` from API when present, else `first_name` + `last_name`), warehouse + track status, **one line** of **main** + **LM** tracking (`order-track-list` preferred, else order-info; not duplicated when both sources match), and **last known location** from tracking events (via STONE3PL-style parsing). The **newest N open orders** are listed (default **N = 50**). A single `order-track-list` batch set covers those ids **plus** up to **50** delivered candidates (newest by `date_added`) used for section 4.
...
4. **Last 5 delivered** (same `orders-info` window): **order id**, **recipient** (same rules as section 1), and **when** — prefer the latest tracking event whose description mentions “delivered”; else the newest timeline event if track status is Delivered (121); else `listed <date_added>`.
...
- **Response (every run):** `preview` is the message body (up to 4000 chars) so production matches what you see in a dry run; `previewWasTruncated` if longer. When Slack runs successfully, `slackPosted: true`. `meta` includes `deliveredInWindowCount` and `deliveredSectionShown` (section 4 rows).
...
N/A (Slack message). Open-order section capped by `WAREHOUSE_SLACK_OPEN_ORDER_LIMIT` (default 50); section 4 shows five delivered rows; total message length trimmed near 3500 chars.
...
- `npx jest tests/daily-slack-summary-tracking.test.ts` — `formatTrackingNumbersForSlack` (single main + LM, track-list vs order fallback).
- `npx jest tests/delivered-when-slack.test.ts` — `resolveDeliveredWhenForSlack` (delivered-event time vs `date_added` fallback).

**Manual**

1. **Dry run (local or preview):** Ensure `CHINADIVISION_API_KEY` is set. `NODE_ENV=development` skips cron auth, or use `Authorization: Bearer <CRON_SECRET>` in production-like mode. Runtime is mostly `orders-info` + inventory + **`order-track-list`** batches (up to ~3 when open cap 50 and up to 50 delivered ids are merged, 40 ids per batch).
   ```bash
...
- **PII:** Recipient names are included in sections **1** and **4**; post only to channels your policy allows (private ops channel + Incoming Webhook scope).
...
- **Section 4 “last 5 delivered”** is chosen from up to **50** delivered rows with the **newest `date_added`**, then sorted by inferred delivery time from tracking; a delivery that completed recently on an **older** platform order (outside that 50) may not appear.

## Performance / monitoring

- Logs: `[cron/warehouse-daily-slack]` with `slackPosted`, counts, `openOrderCap`, `deliveredInWindowCount`, `deliveredSectionShown`, and `slackCharCount`.
...
| Date | Change |
|------|--------|
| 2026-04-04 | **Section 4:** last **5** delivered orders — recipient + inferred delivery time; `order-track-list` batch merges open ids + up to 50 delivered candidates. |
| 2026-04-04 | Open-order tracking: **one** `main` + `LM` line (prefer `order-track-list`, else order-info; removed duplicate Local/STONE3PL). |
