...
- Updated: 2026-03-27 — **Picker Street ladder UI** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx): when ladder data exists, **next-step** copy (`N more · then $X` / `N more · edition ends`) sits on the **image** bottom; [`EditionBadgeForProduct`](../../../app/(store)/shop/experience-v2/components/EditionBadge.tsx) `chipOnly` is in the **footer** under list price. Non-ladder cards keep the edition chip on the image only. Early-access strikethrough unchanged. [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) does not show ladder captions on thumbnails.
