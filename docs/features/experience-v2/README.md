...
- Updated: 2026-04-06 — **Default spotlight skip list**: [`artist-spotlight/route.ts`](../../../app/api/shop/artist-spotlight/route.ts) defines `DEFAULT_SPOTLIGHT_SKIP_VENDORS` (e.g. **`saturn_png`**) so automatic Season 2 / “newest Shopify product” resolution does **not** feature that vendor; the next eligible artist wins. **`?artist=`** / **`?vendor=`** deep links are unchanged.
