...
- **Mini 3D strip tile** — first slot in **collection** mode: **smaller** than artwork thumbs (`w-14` vs `w-24`), **on by default**. **Trash** / **+** hide or show the tile; visibility in **`sessionStorage`** — [`experience-carousel-mini-spline.ts`](../../../lib/shop/experience-carousel-mini-spline.ts). Tap the thumb: `scrollIntoView` on [`data-experience-reel-spline-section`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx) + `onJumpToSpline`. **Same lamp scene as the main reel:** [`CarouselStripLampSpline`](../../../app/(store)/shop/experience/components/CarouselStripLampSpline.tsx) mounts [`Spline3DPreview`](../../../app/template-preview/components/spline-3d-preview.tsx) with the **same** `scene.splinecode`, panel UUIDs, `swapLampSides` / flip / image-position locals as [`SplineFullScreen`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx) (second WebGL instance; deferred `requestIdleCallback` ~5.5s after load). **Turntable** uses Spline **`idleSpinEnabled`** / **`animate`** (not CSS). If `miniSplineLampPreview` is omitted, optional **embed** env or CSS **`.experience-carousel-mini-turntable`** + facade still apply as fallback.
