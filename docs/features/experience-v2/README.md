...
- Updated: 2026-03-27 — **Picker Street ladder UI** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx): image [`StreetPricingChip`](../../../app/(store)/shop/experience-v2/components/StreetPricingChip.tsx) **stage only** (`showPrice={false}`, `showSubcopy={false}`); footer has **list price**, a **scarcity chip** (`subcopy`), and a muted **→ next list price** hint from `nextBump`. Early-access strikethrough uses ladder reference.
