...
Experience V2 is a redesigned artwork selection flow with a cleaner, more immersive interface. The 3D Spline lamp takes up the main viewport, artwork selection happens in an Instagram-style slide-up sheet, and selected artworks appear in a tappable carousel — **bottom overlay on `md+`**, **under the experience header (above the reel) on mobile** (`max-md`).
...
└── components/
    ...
    ├── ArtworkCarouselBar.tsx  # Carousel strip: bottom overlay desktop; top-of-content on mobile (`dockCarouselTopOnMobile`); `reserveCheckoutBar` lifts above sticky checkout on md+
...
- Updated: 2026-04-06 — **Mobile carousel above Spline**: Both [`ExperienceV2Client`](../../../app/(store)/shop/experience-v2/components/ExperienceV2Client.tsx) shells wrap reel + strip in a **`max-md:flex-col`** region with flex **order** — **volume discount** (v2 when shown) → **[`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx)** with **`dockCarouselTopOnMobile`** → **[`SplineFullScreen`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx)** (`max-md:relative` + **`flex-1`**). Desktop keeps **absolute** full-viewport Spline + bottom-docked strip.
