...
- `ExperienceCheckoutStickyBar` — fixed bottom bar: **empty collection** (`stripMode === 'collection'`, no artworks yet) shows only **“Choose your first artwork”** (blue CTA, opens picker; no checkout on this row — use header cart if needed). **≥1 artwork:** thumbnails in cart order at **all breakpoints** — **one tile per `lampQuantity`** (0 = no lamp tile) **+** each selected artwork, `+` separators between tiles, overflow `+N`. **Checkout · $total →** and **centered overlay** glass add FAB (`onOpenPicker`). Thumbnails match [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) **`aspect-[14/20]`** / **`rounded-[15px]`**. **Mobile** (narrow viewport, sticky strip visible — not while `suppressCartThumbnails`): tapping a thumbnail runs the same **Spline / carousel selection** as tapping the strip (`onSelectThumbnailForSpline` → `handleTapCarouselItem` / `carouselSlotIndexForProductId` or legacy `activeStripProducts` index); **desktop** lamp tile can still open lamp detail via `onViewLampDetail`. **Watchlist** with zero artworks does **not** render this bar. [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) sets `reserveCheckoutBar` when the sticky bar is present (empty collection or ≥1 artwork) so the strip clears the bar; carousel **choose** CTA and strip **+** stay off when reserved
...
- Updated: 2026-04-06 — **Mobile sticky thumbnails → Spline**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) **`onSelectThumbnailForSpline`** + **`previewSelectedProductId`**; both [`ExperienceV2Client`](../../../app/(store)/shop/experience-v2/components/ExperienceV2Client.tsx) shells pass them when **`isMobile && !splineInView`** so checkout-bar taps mirror carousel selection (ring + `aria-current` on the active tile).
