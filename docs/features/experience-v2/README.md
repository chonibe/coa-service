...
- **FilterPanel** — artists, tags, price range, sort, in-stock, star rating (reused from V1). **`productsForFilterPanel`** is the **union of Season 1 + Season 2** (deduped by Shopify product id) so artist checkboxes list everyone with catalog works in either season, not only the active tab.
...
- Updated: 2026-04-06 — **Bundle CTA placement**: Same offer as Filters — [`ArtistSpotlightBanner`](../../../app/(store)/shop/experience-v2/components/ArtistSpotlightBanner.tsx) (collapsed row + expanded card in picker / Configurator grid) and [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) on the main experience (above “Choose your first artwork” or above the checkout strip when the bundle is not yet active).
