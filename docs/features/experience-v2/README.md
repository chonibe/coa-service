...
- Updated: 2026-03-27 — **Picker Street ladder UI** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx): [`StreetPricingChip`](../../../app/(store)/shop/experience-v2/components/StreetPricingChip.tsx) at the **bottom** of the image well, **stage only** (`showPrice={false}`, `showSubcopy={false}`); meta **footer** has ladder **list price** and one chip from `nextBump`: **`N more sales · then $X`** (or **`N more sales · edition ends`** at final rung). Early-access strikethrough uses ladder reference. [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) does not show ladder captions on thumbnails.
