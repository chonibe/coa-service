...
- Updated: 2026-03-27 — **Picker card footer price** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx) uses **ladder `priceUsd`** when edition-states data is present (same reference as [`StreetPricingChip`](../../../app/(store)/shop/experience-v2/components/StreetPricingChip.tsx)); early-access strikethrough uses that reference, not storefront-only.
