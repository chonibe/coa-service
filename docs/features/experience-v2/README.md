...
- **Picker entry:** When the collection is **empty**, a **large centered labeled CTA** (“Choose your first artwork” + chevron) opens the sheet — high-contrast blue pill, full visible label (not an icon-only control). Once at least one artwork is selected, a compact **glass +** (`backdrop-blur-xl`, translucent fill, light border) sits **fixed on the right** of the bar: it is **outside** the horizontal scroll container so it stays visible while thumbnails scroll, visually **after** the carousel. The scroll strip holds **only** selected artworks and spotlight placeholders (no add tile inside the row)
...
- Updated: 2026-03-28 — [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx): when the collection is non-empty, the picker **glass +** sits **fixed on the right** (sibling of the scroll strip, not inside it) so it stays visible while thumbnails scroll.
...
- Updated: 2026-03-27 — **Picker ladder without Supabase row**: [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx) uses [`streetEditionRowFromStorefrontProduct`](../../../lib/shop/street-edition-from-storefront.ts) when [`/api/shop/edition-states`](../../../app/api/shop/edition-states/route.ts) omits a product (no `products` row): `custom.edition_size` + variant `quantityAvailable`, with Season 1/2 band defaults (90 / 44) when the metafield is missing but inventory is tracked — so footer **N more · then $X** replaces image [`EditionBadge`](../../../app/(store)/shop/experience-v2/components/EditionBadge.tsx) when the API map is empty for that id.
