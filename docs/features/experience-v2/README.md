...
- **Mini 3D strip tile** — **collection** mode, tied to **lamp cart** (`lampQuantity`): **`lampQuantity > 0`** → first-slot **w-14** thumb with same [`CarouselStripLampSpline`](../../../app/(store)/shop/experience/components/CarouselStripLampSpline.tsx) / [`Spline3DPreview`](../../../app/template-preview/components/spline-3d-preview.tsx) as the main reel; **trash** calls `onRemoveLampFromCarouselStrip` (typically `lampQuantity → 0`). **`lampQuantity === 0`** and **empty collection** (no selected artworks): **spotlight-style** lamp tile (**w-24**, top spacer + bottom glass chip with **+**) matches featured-print placeholders; tap runs `onAddLampFromCarouselStrip` (add lamp). **No lamp + artworks in cart** → mini lamp tile **hidden** (only prints in the strip). Tap in-cart thumb: `scrollIntoView` on [`data-experience-reel-spline-section`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx) + `onJumpToSpline`. Optional embed / CSS turntable fallback if `miniSplineLampPreview` omitted — [`experience-carousel-mini-spline.ts`](../../../lib/shop/experience-carousel-mini-spline.ts)
...
**Product video + photos:** When the product has **Shopify video** (or external embed), [`ProductStandaloneVideoEmbed`](../../../app/(store)/shop/experience-v2/components/ProductStandaloneVideoEmbed.tsx) renders it **outside** the swipe carousel. **Native** Shopify `VIDEO` uses **[`shopifyVideoPlaybackUrl`](../../../lib/shop/product-carousel-slides.ts)** — same rule as PDP [`ProductGallery`](../../../app/(store)/shop/[handle]/components/ProductGallery.tsx): **one** progressive URL (**tallest** `height` among MP4/MOV/WebM). **Progressive** playback matches the home hero [`VideoPlayer`](../../../components/sections/VideoPlayer.tsx): **`preload="none"`**, **~250ms deferred `src`** (poster paints first), plus **`<source type="…">`** via [`shopifyMimeTypeForPlaybackUrl`](../../../lib/shop/product-carousel-slides.ts). **HLS-only** sources still use **`HlsOrSingleUrlVideo`**. External hosts use **iframe** (`embedUrl` / `embeddedUrl`). [`ProductDetailCarousel`](../../../app/(store)/shop/experience-v2/components/ProductDetailCarousel.tsx) is **images only** ([`splitProductCarouselMediaSlides`](../../../lib/shop/product-carousel-slides.ts) splits slides from [`buildProductCarouselSlides`](../../../lib/shop/product-carousel-slides.ts)).

**Implementation:** [`ArtworkDetail.tsx`](../../../app/(store)/shop/experience-v2/components/ArtworkDetail.tsx) · [`ProductStandaloneVideoEmbed.tsx`](../../../app/(store)/shop/experience-v2/components/ProductStandaloneVideoEmbed.tsx) · [`ProductDetailCarousel.tsx`](../../../app/(store)/shop/experience-v2/components/ProductDetailCarousel.tsx) · [`product-carousel-slides.ts`](../../../lib/shop/product-carousel-slides.ts) · [`LampIncludesSpecsPanel.tsx`](../../../app/(store)/shop/experience-v2/components/LampIncludesSpecsPanel.tsx) · [`EditionWatchWithNarrative.tsx`](../../../app/(store)/shop/experience-v2/components/EditionWatchWithNarrative.tsx) · [`EditionBadge.tsx`](../../../app/(store)/shop/experience-v2/components/EditionBadge.tsx) (picker/strip/chip contexts) · [`edition-stages.ts`](../../../lib/shop/edition-stages.ts)

**Tests:** [`product-carousel-slides.test.ts`](../../../lib/shop/product-carousel-slides.test.ts) (`shopifyVideoPlaybackUrl`, slides, external `embedUrl`). Manually verify playback on a product with native video and with external embed.
...
- **OrderBar** opens a drawer with the order summary; **Continue to secure checkout** calls [`POST /api/checkout/create`](../../../app/api/checkout/create/route.ts) (same hosted flow as the [cart page](../../../app/(store)/shop/cart/page.tsx)). Shipping address and payment are completed on **Stripe’s hosted Checkout** page, not embedded Payment Element in the drawer. `cancelUrl` returns to the current experience path with `?cancelled=true`. Promo codes from the slideout menu are sent as `promoCode`; authenticated users pass `customerEmail` for Stripe prefill. Implementation: [`OrderBar.tsx`](../../../app/(store)/shop/experience-v2/components/OrderBar.tsx). Funnel pulse for the primary CTA uses **`stripeHostedInDrawer`** in [`resolveExperienceNextAction`](../../../lib/shop/experience-journey-next-action.ts) so `place_order` highlights when the drawer is open with artworks (no separate address/payment steps in-app).
...
- Updated: 2026-03-27 — **Cart / checkout artwork USD** matches the Street ladder from [`/api/shop/edition-states`](../../../app/api/shop/edition-states/route.ts): [`experienceArtworkUnitUsd`](../../../lib/shop/experience-artwork-unit-price.ts) in [`OrderBar`](../../../app/(store)/shop/experience-v2/components/OrderBar.tsx) (lock → ladder → storefront) drives line subtotals and header totals in both [`ExperienceV2Client`](../../../app/(store)/shop/experience-v2/components/ExperienceV2Client.tsx) entry points; hosted checkout re-applies ladder server-side in [`POST /api/checkout/create`](../../../app/api/checkout/create/route.ts).
...
- Updated: 2026-04-06 — **Native video = PDP playback rule**: [`shopifyVideoPlaybackUrl`](../../../lib/shop/product-carousel-slides.ts) + single **`<video src>`** in [`ProductStandaloneVideoEmbed`](../../../app/(store)/shop/experience-v2/components/ProductStandaloneVideoEmbed.tsx); PDP [`convertShopifyMedia`](../../../app/(store)/shop/[handle]/components/ProductGallery.tsx) uses the same helper; **external** iframe uses **`embedUrl` ?? `embeddedUrl`** on PDP too.
- Updated: 2026-04-06 — **Native video = home `VideoPlayer` load pattern**: deferred **`src`**, **`preload="none"`**, typed **`<source>`** ([`shopifyMimeTypeForPlaybackUrl`](../../../lib/shop/product-carousel-slides.ts)) in [`ProductStandaloneVideoEmbed`](../../../app/(store)/shop/experience-v2/components/ProductStandaloneVideoEmbed.tsx).
...
- Updated: 2026-04-06 — **Strip mini = main Spline scene**: [`CarouselStripLampSpline`](../../../app/(store)/shop/experience/components/CarouselStripLampSpline.tsx) + `miniSplineLampPreview` from both experience clients — same [`Spline3DPreview`](../../../app/template-preview/components/spline-3d-preview.tsx) config as [`SplineFullScreen`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx); embed/facade path remains fallback when the prop is not passed.
- Updated: 2026-04-06 — **Strip mini + lamp cart**: [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) shows live mini Spline only when **`lampQuantity > 0`**; empty collection + no lamp uses spotlight-style **+** tile (`stripLampProduct`, `onAddLampFromCarouselStrip`); trash uses `onRemoveLampFromCarouselStrip`. Removed strip visibility **sessionStorage** from [`experience-carousel-mini-spline.ts`](../../../lib/shop/experience-carousel-mini-spline.ts).
- Updated: 2026-04-06 — **Strip mini lamp (empty panels)**: [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) passes **`image1`/`image2` null** and zero preview counts into [`CarouselStripLampSpline`](../../../app/(store)/shop/experience/components/CarouselStripLampSpline.tsx) so the carousel thumb shows the **base lamp** only; the main viewport [`SplineFullScreen`](../../../app/(store)/shop/experience/components/SplineFullScreen.tsx) still uses cart artwork textures.
- Updated: 2026-04-06 — **Sticky checkout vs carousel**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) **`suppressCartThumbnails`** (wired from **`splineInView`** in both [`ExperienceV2Client`](../../../app/(store)/shop/experience-v2/components/ExperienceV2Client.tsx) shells) hides the **thumbnail strip** while the Spline + bottom carousel are in view; **add-artwork FAB** and **Checkout · $** stay. After scrolling past the reel, full thumbs + checkout row returns.
- Updated: 2026-04-06 — **Mobile sticky thumbnails → Spline**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) **`onSelectThumbnailForSpline`** + **`previewSelectedProductId`**; both [`ExperienceV2Client`](../../../app/(store)/shop/experience-v2/components/ExperienceV2Client.tsx) shells pass **`onSelectThumbnailForSpline`** when **`isMobile`** so checkout-bar taps mirror carousel selection (ring + `aria-current` on the active tile).
- Updated: 2026-04-07 — **Hosted Stripe Checkout (experience + gift cards)**: [`OrderBar`](../../../app/(store)/shop/experience-v2/components/OrderBar.tsx) uses [`POST /api/checkout/create`](../../../app/api/checkout/create/route.ts) + redirect to `session.url` (no embedded `PaymentStep` / `create-checkout-session` in the drawer). [`stripeHostedInDrawer`](../../../lib/shop/experience-journey-next-action.ts) drives journey pulse. Gift cards: [`POST /api/gift-cards/create-checkout`](../../../app/api/gift-cards/create-checkout/route.ts) returns `url`; [`gift-cards/page.tsx`](../../../app/(store)/shop/gift-cards/page.tsx) redirects; cancel banner on `?cancelled=true`. Removed embedded gift card drawer (`GiftCardCheckoutDrawer.tsx`); checkout is redirect-only.
- Updated: 2026-04-06 — **Mobile: no artwork carousel strip**: [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) is wrapped with **`hidden md:block`** in both experience clients; **cart thumbnails** live only on [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) below **`md`**. **`suppressCartThumbnails`** is **`!isMobile && splineInView`**; **`onSelectThumbnailForSpline`** runs for **`isMobile`** whenever the bar shows thumbs.
- Updated: 2026-04-06 — **Sticky “on lamp” eyes**: [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) takes **`lampPreviewProductIds`** (same as **`lampPreviewOrder`**); **artwork** thumbnails show the eye **above** the tile (layout row, not clipped) for **each** id assigned to a lamp side.
...
- Updated: 2026-04-08 — **Collection + pulse**: [`globals.css`](../../../app/globals.css) **`animate-experience-collection-plus-prize-pulse`** (violet glow + brightness) and **`animate-experience-collection-plus-prize-float`** (subtle vertical motion) on the **carousel strip** [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) **+** only; sticky [`ExperienceCheckoutStickyBar`](../../../app/(store)/shop/experience-v2/components/ExperienceCheckoutStickyBar.tsx) picker FAB stays static (journey shine / checkout flash unchanged). Disabled when **`prefers-reduced-motion: reduce`**.
