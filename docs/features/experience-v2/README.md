...
- Updated: 2026-03-27 — **Picker Street ladder UI** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx): bottom-of-image chip is always [`EditionBadgeForProduct`](../../../app/(store)/shop/experience-v2/components/EditionBadge.tsx) `chipOnly` (consistent stage icons); ladder API drives the **footer** list price, next-step chip from `nextBump`, and early-access strikethrough. [`ArtworkCarouselBar`](../../../app/(store)/shop/experience/components/ArtworkCarouselBar.tsx) does not show ladder captions on thumbnails.
