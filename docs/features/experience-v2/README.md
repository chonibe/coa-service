...
- **Unselected** — image + footer use the same **`bg-white` / `bg-[#171515]`** as the sheet. **Selected** — **light**: full **`#f0f9ff`** on shell, image well, and title row. **Dark**: outer shell stays **untinted**; image well uses base **`#171515`** with a **bottom-half** wash (`#2c2828` / 90%) over the image area; **footer/meta** stays full **`#2c2828`** (no ring). [`ArtworkStrip`](../../../app/(store)/shop/experience-v2/components/ArtworkStrip.tsx) **single** in-cart tiles use **`#e8f4ff` / `#1a1616`** shell tint only (**no** outer border). **Merged same-vendor pair** (picker + strip): tinted **`#f0f9ff` / `#2c2828`** row when both selected; **no** outer border on 2-up rows; artist **spine** is text-only (no spine bg/borders). Footers: **centered** title + price; strip cards **no** border between image and meta
...
- Updated: 2026-03-20 — Picker **dark** selected: selected tint on image is **lower half only** (overlay in [`ArtworkPickerSheet.tsx`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx)); shell transparent in dark when selected.
- Updated: 2026-03-20 — Artwork cards: **no** peach ring / inset glow on strip in-cart; **no** `border-2` placeholder on picker + strip card shells.
