...
- Updated: 2026-03-27 — **Picker Street ladder UI** in [`ArtworkPickerSheet`](../../../app/(store)/shop/experience/components/ArtworkPickerSheet.tsx): [`StreetPricingChip`](../../../app/(store)/shop/experience-v2/components/StreetPricingChip.tsx) on the image with `showPrice={false}` (stage + subcopy only); **list price** and **next bump** line in the footer (`nextBump` from [`GET /api/shop/edition-states`](../../../app/api/shop/edition-states/route.ts)). Early-access strikethrough uses ladder reference.
