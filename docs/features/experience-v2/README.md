...
- **Featured artist bundle**: [`FeaturedArtistBundleSection`](../../../app/(store)/shop/experience/components/FeaturedArtistBundleSection.tsx) below the 3D preview when the offer exists and either the **collection is empty** (first-visit promo) or the **bundle CTA is still actionable** (`offer.disabled === false` — e.g. user has other art but has not yet locked in the spotlight trio).
