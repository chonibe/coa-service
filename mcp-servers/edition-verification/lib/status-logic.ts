import type { ShopifyOrder, ShopifyLineItem, LineItemStatusResult } from './types.js';
 ...
 */
export function determineLineItemStatus(
  order: ShopifyOrder,
  lineItem: ShopifyLineItem
): LineItemStatusResult {
  ...
  // REFUND DETECTION
  const isRefunded = 
    removedLineItemIds.has(liIdStr) || 
  ...
  // REMOVED BY PROPERTY
  const removedProperty = lineItem.properties?.find((p) => 
  ...
  const isRemovedByQty = 
    (lineItem.fulfillable_quantity === 0 || lineItem.fulfillable_quantity === '0') && 
  ...
  const status: 'active' | 'inactive' = isInactive ? 'inactive' : (isPaid || isFulfilled ? 'active' : 'inactive');
 ...
 */
export function getRefundedLineItemIds(order: ShopifyOrder): Set<string> {
 ...
 */
export function isOrderCancelled(order: ShopifyOrder): boolean {
  return (
