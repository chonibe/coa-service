...
import { determineLineItemStatus, getRefundedLineItemIds } from "./lib/status-logic.js";
import { getFilteredCollectorEditions } from "./lib/collector-editions.js";
    ...
    try {
        switch (name) {
            ...
            case "sync_order_line_items": {
                const { order, skip_editions = false } = args;
            ...
            case "mark_line_item_inactive": {
                const { line_item_id, reason, notes } = args;
                // Get current state
          ...
          `)
            ...
            case "validate_data_integrity": {
                const { product_id, collector_id } = args;
                const issues = [];
          ...
          `);
                ...
                if (product_id) {
                    const activeItems = (lineItems || []).filter((li) => li.status === 'active' && li.edition_number !== null);
