...
async function getTableRelationships(tableName: string) {
  const query = `
    SELECT
      tc.table_name,
      kcu.column_name,
  ...
  try {
    switch (name) {
      ...
      case "query_relationships": {
        ...
        const reverseRelationships: any[] = [];
      ...
      case "test_query": {
        ...
        if (!isReadOnlyQuery(query)) {
          throw new Error(
            "Only SELECT queries are allowed. Write operations (INSERT, UPDATE, DELETE, etc.) are blocked for safety."
          );
        }
